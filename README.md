...
正本スキルは `.cursor/skills/product-video/` です。起動は `/product-video`。通常確認は `案Nで台本OK` と `完成・格納してください` だけ。完成後の既定は、型番名の Drive フォルダへの新規格納です。
...
6. Cursorでこのリポジトリを開き、実行先をこの Mac の Desktop Agent にする。`/product-video` で新規案件を開始する。まず `案Nで台本OK` まで進めて止める。
