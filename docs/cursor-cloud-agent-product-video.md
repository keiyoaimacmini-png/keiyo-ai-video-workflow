...
一括のホリデーツイスト生成では、各台本行のあいだに測った無音を入れてから、その無音でシーンごとに切ります。結合した1本のナレーションのまま尺を合わせません。最終テロップは画面中央で、はみ出す行は見た目だけ改行します。
...
Cloud AgentがChromeまたはCapCut Webを実際に操作・確認できる連携を持つ場合、または同じ証拠契約を満たすホスト編集アダプタがある場合だけ、編集工程へ進みます。連携がない場合は、台本と検証済みhandoffまでは作れても、実編集を完了したとは扱いません。1案件で2つのエディタを混ぜません。
