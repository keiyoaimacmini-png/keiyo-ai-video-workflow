...
| `HOLD_GEMINI_WEB_NOT_VERIFIED` | Official Gemini Web origin `https://gemini.google.com/` or a usable Chrome browser adapter is not verified **on this host** for the Checkpoint 1 script draft. A session on a Cloud Agent VM does not prove the operator Mac, and the reverse is also false. Do not copy cookies. Do not call the Gemini API as a substitute. Leave the rendered paste prompt in the task file so the operator can paste it in this machine's Chrome. |
| `HOLD_GEMINI_LOGIN_USER_ACTION_REQUIRED` | Gemini Web login on **this host's Chrome** needs ambiguous account choice, CAPTCHA, 2FA, recovery, new consent, or credential handling beyond the existing session or saved autofill. Same pattern as CapCut. If the current host is Cloud Agent, continue on Cursor Desktop on the physical machine. If the current host is already that Desktop, the operator uses this machine's Chrome. Never paste passwords or API keys. |
