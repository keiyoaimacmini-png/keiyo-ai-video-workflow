# Three-checkpoint contract

These are the only routine user gates. Present the exact review location and open it when supported.
...
- narration-target count `N`;
- initial-generation ceiling `N`;
- repair-generation reserve `N`, limited to one per cut;
- total generation-action ceiling `2N`;
- no credit purchase;
