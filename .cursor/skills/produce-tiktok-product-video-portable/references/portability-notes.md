...
- Gemini.app script draft on this Mac at Gemini 3.8 Flash, with no API key and no Cursor parent-model switch for that draft;
...
- completed-video 格納 through `scripts/upload_drive_local_file.py` plus adapter read-back, never a Chrome screenshot loop;
