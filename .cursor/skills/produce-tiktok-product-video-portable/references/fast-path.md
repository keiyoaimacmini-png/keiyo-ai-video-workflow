...
- Confirm the approved payload timing against the actual rough timeline. If source, range, visible action, wording, line breaks, or voice identity must change, preserve the current evidence and return to a revised Checkpoint 1; never rebind the existing `台本OK`. A later settings-bounded common-speed adjustment is a finish-time value, not a Checkpoint 1 reopen trigger.
...
- Generate official Holiday Twist immediately. If the editor of record cannot emit that preset, use the CapCut Text to Speech sidecar (no picture, no substitute voice, no new case). Do not pause for a Path 1 / Path 2 choice.
...
- If pre-authorized by `完成・書き出しOK` and `delivery_mode: drive`, upload once from local bytes into the folder titled with this product model. Do not inline the video as base64. If local-path ingest is unavailable, use one authenticated Drive UI upload into the proven parent, then adapter read-back. Do not copy into Downloads unless the picker cannot see `out/`.
...
- Do not pause after `粗編集OK` for Path 1 (substitute voice) or Path 2 (new CapCut case).
...
- Do not inline the completed video as base64 in a Drive create. Skip that path; use local-byte ingest or one authenticated UI upload plus read-back.
