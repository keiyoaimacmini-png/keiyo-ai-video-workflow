...
If login is lost, use only the official site and an already-authorized browser session or saved autofill. Do not display, read, copy, log, or export credentials, cookies, or tokens. For CapCut or TikTok, account ambiguity, CAPTCHA, 2FA, recovery, or new consent requires `HOLD_CAPCUT_LOGIN_USER_ACTION_REQUIRED`. For the Checkpoint 1 script draft, use this Mac's logged-in Gemini.app. Do not use Google Chrome.app or the agent-controlled browser as a substitute. Do not screenshot, OCR, or Accessibility-hunt Gemini.app. Do not switch the Cursor parent model to write the script. Select Gemini 3.8 Flash and read that label back; any other picker value is `HOLD_GEMINI_MODEL_NOT_VERIFIED`. Treat login, CAPTCHA, 2FA, recovery, or new consent as `HOLD_GEMINI_LOGIN_USER_ACTION_REQUIRED`. Missing Gemini.app is `HOLD_GEMINI_WEB_NOT_VERIFIED`; restore the logged-in app, send the prompt, and read the dialogue. Do not ask the operator to paste. Write the key-free prompt with `scripts/render_gemini_web_prompt.py` for that send. Do not call the Gemini API for that draft. Never put Google, Gemini, CapCut, or TikTok passwords in repository files or prompts. Completed-video 格納 uses `scripts/upload_drive_local_file.py` in the same turn as the export read-back, plus adapter read-back. Do not open Chrome.app for 格納. For original confirmation or material fetch the Drive adapter cannot do, open only `https://drive.google.com/` in this Mac's Google Chrome.app. Do not screenshot-hunt Chrome.app. Do not use Google Drive for desktop, a local sync mount, rclone, or the agent-controlled browser as a substitute. Missing Chrome Drive origin for a required Web-only step is `HOLD_DRIVE_WEB_NOT_VERIFIED`. Drive login, CAPTCHA, 2FA, recovery, or account choice is `HOLD_DRIVE_LOGIN_USER_ACTION_REQUIRED`.
...
Do not encode the completed video as base64 in a tool argument. Do not attempt a Drive create that inlines completed-video bytes. The local-path ingest is `scripts/upload_drive_local_file.py`: filesystem bytes over HTTPS resumable upload, then adapter read-back. Runtime OAuth belongs only under gitignored `.runtime/`. Do not read host OAuth databases, cookies, or connector secrets. Do not screenshot, OCR, or click-hunt Chrome.app to finish 格納. One already-authenticated Chrome.app Drive Web drop is last-resort only when that picker is already on the proven parent and the helper cannot run. Do not use Google Drive for desktop. Do not create a same-name empty or path-string decoy. If the parent is proven but no local-byte path exists, `HOLD_DRIVE_LOCAL_BYTES_UNAVAILABLE`.
