...
Each Holiday Twist render is one credit-consuming request for exactly one `cut_id`. Append `initial_generation_requested` for that cut only. Do not share one bulk source-file hash across every `cut_id`.
