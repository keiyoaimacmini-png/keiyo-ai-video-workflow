...
| Drive adapter cannot take local completed-video bytes | Confirm parent title, no same-name collision, and that the local export receipt exists | Do not inline the video as base64. Upload once through the already-authenticated Drive UI into that proven parent, then adapter read-back. If that UI path is also unavailable, `HOLD_DRIVE_LOCAL_BYTES_UNAVAILABLE` | New file identity, exact name, MIME, byte size, parent scope, and time match the export |
