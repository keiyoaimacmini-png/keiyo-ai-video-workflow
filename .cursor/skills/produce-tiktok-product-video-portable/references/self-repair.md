...
| Editor of record cannot emit official Holiday Twist | Confirm frozen preset, case editor identity, and that picture has not been imported to CapCut | Generate once per narration-target cut on official CapCut Text to Speech (that cut's frozen line only), capture the result-card audio bytes with `scripts/capture_capcut_result_audio.py` (no オーディオのみ, no save dialog, no operator Save), import each clip into the editor of record. Do not offer a substitute voice, a voice reopen, or a new CapCut case | Official preset audio exists as a working copy in the editor of record |
...
| Final caption missing on the visual tail | Re-read Caption Cards and inspect the last composed frame. `cue_override` JSON is not a hold | Do not clone the same TTS asset as a muted hold. Do not use Motion Graphics as captions. Use a distinct padded speech source only when it stays inside the one-repair cap and frozen line | Last valid frame still shows the frozen CTA line through `timeline_end_frame` |
...
| Export/upload appears stalled | Poll current operation and read back filesystem/Drive | No resubmission. Continue observation only. Do not probe with a same-name empty Drive file | One unambiguous file/Drive record or HOLD unknown outcome |
| Drive adapter cannot take local completed-video bytes | Confirm parent title, no same-name collision, and that the local export receipt exists | Do not inline the video as base64. Run `scripts/upload_drive_local_file.py`. If it HOLDs for OAuth, stop with `HOLD_DRIVE_LOCAL_BYTES_UNAVAILABLE` and give the operator the `--login` command. Do not screenshot-hunt Chrome.app. Do not use Google Drive for desktop | New file identity, exact name, MIME, byte size, parent scope, and time match the export |
