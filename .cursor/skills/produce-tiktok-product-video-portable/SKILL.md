...
`COMPLETE` requires a verified new export receipt. For the default `drive` mode it also requires one verified new Drive object in the folder titled with the verified product model and exact parent-scope read-back. Create that file with `scripts/upload_drive_local_file.py` from local bytes in the same turn as the export read-back; do not inline the video as base64 and do not open Chrome.app for 格納. Close only proven task-owned browser tabs after that read-back. Uncertain tab ownership is `HOLD_TASK_TAB_IDENTITY_UNVERIFIED` and does not block `COMPLETE`. `export_only` still requires a destination-stored receipt that is not a local working copy.
