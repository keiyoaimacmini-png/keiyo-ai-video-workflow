...
def valid_caption_template_requirement(value):
    """Official CapCut templates are optional. Host-editor captions are valid."""
    return isinstance(value, dict) and non_empty_string(value.get("provider"))
...
def exact_closure(records, cut_ids, label, field, allowed, cut_ranges, errors):
    ...
    for index, record in enumerate(records):
        if not isinstance(record, dict) or set(record) != allowed or not isinstance(record.get("cut_id"), str) or not isinstance(record.get(field), str) or not valid_time_range(record.get("timeline_in"), record.get("timeline_out")):
            ...
            if label == "captions" and (record.get("track") != "caption" or not isinstance(record.get("layer"), int) or record.get("position") != "center" or not isinstance(record.get("style"), dict) or set(record["style"]) != {"font", "color", "outline", "shadow", "size", "alignment"} or not isinstance(record.get("line_breaks"), list) or not valid_caption_template_requirement(template_requirement)):
...
def check_routing(payload, production_hash, visible_hash, errors):
    ...
    if routing is None:
        if payload.get("openclaw_prohibition") is not None or payload.get("camee_tiktok_shop") is not None:
            errors.append("routing compatibility extension is required when its receipts are present")
        return
