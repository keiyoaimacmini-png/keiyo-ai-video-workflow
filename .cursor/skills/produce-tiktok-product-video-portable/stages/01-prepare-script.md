...
Send only that rendered prompt into Gemini.app. Never paste, log, or type Google, Gemini, CapCut, or TikTok passwords, cookies, tokens, or API keys. Do not screenshot, OCR, or Accessibility-hunt Gemini.app. Same turn: bring the logged-in Gemini.app forward, send the prompt, and read the dialogue back. Do not leave a paste for the operator. If a method that already worked on this Mac fails, restore it and finish the draft. `HOLD_GEMINI_WEB_NOT_VERIFIED` does not authorize an operator paste. If login, CAPTCHA, 2FA, account choice, recovery, or new consent is required, `HOLD_GEMINI_LOGIN_USER_ACTION_REQUIRED`. Do not ask the user to choose among concepts.
