...
6. Retrieve any project-required generation context. Keep selected reusable patterns and their `not_to_copy` boundaries distinct. In this Cursor Cloud workflow, after the material inventory exists, draft the twenty-candidate comparison and selected dialogue in official Gemini Web chat, the same Chrome-session pattern as CapCut Web. Open only `https://gemini.google.com/`. Prefer Gemini 3.8 Flash in the model picker when that label is visible. Paste a key-free brief of verified facts and observed shot actions only. Never paste, log, or type Google, Gemini, CapCut, or TikTok passwords, cookies, tokens, or API keys. If the host has no browser/editor adapter, or the live origin is not official Gemini Web, `HOLD_GEMINI_WEB_NOT_VERIFIED`. If login, CAPTCHA, 2FA, account choice, recovery, or new consent is required, `HOLD_GEMINI_LOGIN_USER_ACTION_REQUIRED` so the user can operate through Cursor Desktop. Do not call the Gemini API for that draft. Other hosts without Gemini Web still compare twenty executable concepts internally. Do not ask the user to choose among concepts.
