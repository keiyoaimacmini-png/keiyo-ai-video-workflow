...
6. Retrieve any project-required generation context. Keep selected reusable patterns and their `not_to_copy` boundaries distinct. In this Cursor workflow, after the material inventory exists, draft the twenty-candidate comparison and selected dialogue in official Gemini Web chat on **this host's Chrome** (Cloud Agent VM or Cursor Desktop on the operator physical machine). A Chrome session proven on the other machine does not transfer; do not copy cookies, tokens, or profile files. Open only `https://gemini.google.com/`. Start a new empty chat. Prefer Gemini 3.8 Flash in the model picker when that label is visible; otherwise use the visible Flash. Write a key-free brief and render the paste prompt:

```bash
python3 "${SKILL_ROOT}/scripts/render_gemini_web_prompt.py" --brief <task-root>/gemini-web-brief.v1.json
```

Paste only that rendered prompt. Never paste, log, or type Google, Gemini, CapCut, or TikTok passwords, cookies, tokens, or API keys. If this host has no browser adapter, or the live origin is not official Gemini Web, `HOLD_GEMINI_WEB_NOT_VERIFIED` and leave the prompt in the task file so the operator can paste it in this machine's Chrome, then return the dialogue. If login, CAPTCHA, 2FA, account choice, recovery, or new consent is required, `HOLD_GEMINI_LOGIN_USER_ACTION_REQUIRED`. When the current host is Cloud Agent, that HOLD continues on Cursor Desktop on the physical machine; do not treat the VM tab as proof on the Mac. Do not call the Gemini API for that draft. Other hosts without Gemini Web still compare twenty executable concepts internally. Do not ask the user to choose among concepts.
...
Create one `product_video_script_package.v1` containing the closed material manifest and model provenance, selected concept, twenty-candidate comparison summary, project generation-context provenance and `not_to_copy`, verified facts/evidence, complete ordered dialogue, exact punctuation and line breaks, cut IDs, source asset/path and SHA, source in/out, cut duration, Unicode count, estimated read time, settings SHA, active-rule snapshot SHA, and canonical final-cut binding. Persist a key-free Gemini Web draft receipt when that path ran (selected dialogue and concept only). Do not store cookies, tokens, account identifiers, or session URLs.
