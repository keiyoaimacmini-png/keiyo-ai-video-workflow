...
- source-clip identity and actual existence, exact source-audio track/clip mute (not low gain), and first-valid/midpoint/last-valid rendered-frame evidence for every cut. Each observed frame must contain the expected non-black source and exactly one rendered caption layer. The configured final cut's last valid frame is `timeline_end_frame`; a missing caption there is `HOLD_CAPTION_TAIL_NOT_CLOSED`. Mute read-back is structured as the exact control, state `muted`, and null gain value; free text or attenuation values cannot pass. Prefer picture-lane track mute.

Inspect the real player at every caption start, every caption boundary, and the final animation state. A centered coordinate in JSON is not visual proof. Exactly one visible text rendering may exist per cut. Composed frames must show the `product-video-center` look: Dela Gothic One, white fill, thick dark stroke, drop shadow, and no background band. Verify the final CTA position separately.
...
Show the actual editable timeline and review location, voice/common speed, per-cut closure, full playback, duplicate-text result, center/safe-area result, bounded repairs, remaining reserve, and the frozen delivery context. When auditory status is pending, include in this same stop message: the editor link, a Japanese full-listening checklist (有無、聞き取り、字幕一致、頭と尻、重なり、重複、切れ、漏れ), and the request for exact `完成・書き出しOK` after the user plays the whole timeline on this desktop. Stop only for exact `完成・書き出しOK`. Bind that approval to `artifacts.final_qa` and `learning_snapshots.delivery`. It authorizes one new export and, only when already fixed in the original scope, one new Drive upload/read-back. Task-owned tab closure is after verified delivery; unknown tab ownership does not block `COMPLETE`.
