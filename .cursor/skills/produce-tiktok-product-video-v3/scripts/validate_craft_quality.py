...
def apply_lesson_script_gates(
    hook: str, resolution: str, lessons: list[dict[str, Any]]
) -> dict[str, Any] | None:
    for lesson in lessons:
        ...
        if gate.get("kind") == "text_pair":
            held = apply_text_pair_gate(
                hook, resolution, gate, str(lesson.get("defect") or "hook_closed_by_result_looks")
            )
            if held:
                return held
    return None
...
def gate_script(artifact: Any, lessons: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    held = apply_lesson_script_gates(hook, resolution, lessons)
    if held:
        return held
...
def gate_tts(artifact: Any, lessons: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if artifact.get("combined_clip_left_on_timeline") is True:
        return hold_payload(
    ...
    for cut in cuts:
        ...
        if cut.get("too_fast_to_hear") is True:
            return hold_payload(f"{cut_id}: narration is too fast to hear", surface="tts-timing", defect="too_fast_to_hear")
...
def self_test(project_root: Path | None) -> int:
    ...
    if project_root is not None:
        ...
        check("seed-lesson-loaded", any(item.get("id") == "hook-closed-by-result-looks" for item in lessons))
    ...
    tts_bad = validate(
        "tts-timing",
        {"schema": "product_video_tts_craft.v1", "combined_clip_left_on_timeline": True, "cuts": [{"cut_id": "cut-01", "three_layer_closed": True}]},
        [],
    )
