...
- Official CapCut text templates, including キラキラ変身, are optional. Use the case editor's caption program (ChatCut Caption Cards or CapCut native captions) for viewer-facing captions. Do not use Motion Graphics as the caption layer. Those captions are valid when they are centered, non-duplicated, fully inside the visible safe area, and prominent (heavy weight, thick dark stroke, contrast band).
